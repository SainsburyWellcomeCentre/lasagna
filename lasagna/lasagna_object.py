...
class Lasagna(QtGui.QMainWindow, lasagna_mainWindow.Ui_lasagna_mainWindow):
    def __init__(self, embed_console=False, parent=None):
        ...
        # These are the graphics_views from the UI file lasagna_mainWindow.py which is generated by
        # lasagna_mainWindow.ui. They are pyqtgraph.PlotWidget
        self.graphicsViews = [
            self.graphicsView_1,
            self.graphicsView_2,
            self.graphicsView_3,
        ...
        for i in range(len(self.graphicsViews)):
            self.axes2D.append(lasagna_axis.projection2D(self.graphicsViews[i], self,
        ...
        # Establish links between projections for scrolling through slices
        # [implemented by signals in main() after the GUI is instantiated]
        ...
        [self.markerSymbol_comboBox.addItem(pointType) for pointType in preferences.readPreference("symbolOrder")]
        ...
        for plugin in plugins:
            ...
            self.pluginActions[plugin_name].setCheckable(True)
            # so we have a checkbox next to the menu entry

            self.pluginSubMenus[dir_name].addAction(self.pluginActions[plugin_name])
            # add action to the correct plugins sub-menu
            self.pluginActions[plugin_name].triggered.connect(self.startStopPlugin)
    ...
    def startStopPlugin(self):
        plugin_name = str(self.sender().objectName())
    ...
    def showStackLoadDialog(self, triggered=None, fileFilter=image_stack_loader.image_filter()):
        ...
        fnames = self.showFileLoadDialog(fileFilter=fileFilter, multiple_selection=True)
    ...
    def showFileLoadDialog(self, fileFilter="All files (*)", multiple_selection=False):
        """
        ...
        if multiple_selection:
            ...
            fnames = QtGui.QFileDialog.getOpenFileName(self, 'Open file',
                                                       preferences.readPreference('lastLoadDir'),
                                                       fileFilter)[0]
            fnames = [fnames]
