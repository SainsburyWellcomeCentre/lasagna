...
def findPlugins(pluginPaths):
    ...
    for thisPath in pluginPaths:
        ...
        thesePluginFiles = [ f for f in listdir(thisPath) if path.isfile(path.join(thisPath,f)) & f.endswith('_plugin.py') ]
        if len(thesePluginFiles) == 0:
            continue
