...
import \
    lasagna_helperFunctions as lasHelp  # Module the provides a variety of import functions (e.g. preference file handling)
...
class lasagna(QtGui.QMainWindow, lasagna_mainWindow.Ui_lasagna_mainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.graphicsViews = [self.graphicsView_1, self.graphicsView_2,
                              self.graphicsView_3]  # These are the graphics_views from the UI file
        self.axes2D = []
        print("")
        for ii in range(len(self.graphicsViews)):
            self.axes2D.append(lasagna_axis.projection2D(self.graphicsViews[ii], self,
                                                         axisRatio=float(self.axisRatioLineEdits[ii].text()),
                                                         axisToPlot=ii))
        print("")
        ...
        self.imageStackLayers_TreeView.selectionModel().selectionChanged[
            QtCore.QItemSelection, QtCore.QItemSelection].connect(self.imageStackLayers_TreeView_slot)
        ...
        self.points_TreeView.selectionModel().selectionChanged[QtCore.QItemSelection, QtCore.QItemSelection].connect(
            self.pointsLayers_TreeView_slot)

        # Settings boxes, etc, for the points (sparse data) ingredients
        [self.markerSymbol_comboBox.addItem(pointType) for pointType in
         lasHelp.readPreference('symbolOrder')]  # populate with markers
        ...
        for thisPlugin in plugins:
            ...
            self.pluginSubMenus[dirName].addAction(
                self.pluginActions[pluginName])  # add action to the correct plugins sub-menu
            self.pluginActions[pluginName].triggered.connect(
                self.startStopPlugin)  # Connect this action's signal to the slot
    ...
    def loadImageStack(self, fnameToLoad):
        ...
        self.addIngredient(object_name=objName,
                           kind='imagestack',
    ...
    def showStackLoadDialog(self, triggered=None, fileFilter=imageStackLoader.imageFilter()):
        ...
        fname = self.showFileLoadDialog(
            fileFilter=fileFilter)  # TODO: this way the recently loaded files are updated before we succesfully loaded
        if fname is None:
            return
    ...
    def quitLasagna(self):
        ...
        for thisPlugin in list(self.pluginActions.keys()):
            if self.pluginActions[thisPlugin].isChecked():
                if not self.plugins[
                    thisPlugin].confirmOnClose:  # TODO: handle cases where plugins want confirmation to close
    ...
    def addIngredient(self, kind='', object_name='', data=None, fname=''):
        """
        ...
        print(("\nlasanga.addIngredient - Adding %s ingredient: %s" % (kind, object_name)))
        ...
        self.removeIngredientByName(object_name)
    ...
    def removeIngredientByType(self, ingredientType):
        ...
        for thisIngredient in self.ingredientList[:]:
            if thisIngredient.__module__.endswith(
                    ingredientType):  # TODO: fix this so we look for it by instance not name
                if verbose:
    ...
    def initialiseAxes(self, resetAxes=False):
        ...
        [axis.updatePlotItems_2D(self.ingredientList, sliceToPlot=axis.currentSlice, resetToMiddleLayer=resetAxes) for
         axis in self.axes2D]
    ...
    def mouseMoved(self, evt):
        ...
        if self.axes2D[axisID].view.sceneBoundingRect().contains(pos):
            ...
            if QtGui.QApplication.keyboardModifiers() == QtCore.Qt.ControlModifier and self.axes2D[
                axisID].view.getViewBox().controlDrag:
...
def main(imStackFnamesToLoad=None, sparsePointsToLoad=None, linesToLoad=None, pluginToStart=None, embedConsole=False):
    ...
    for ii in range(3):
        ...
        thisProxy = pg.SignalProxy(tasty.axes2D[ii].view.getViewBox().mouseClicked, rateLimit=30,
