"""
Read tree structures from a text file. This reader is very similar to the line reader. 
The data format is:
...
class loaderClass(LasagnaPlugin):
    def __init__(self, lasagna):
        ...
        iconLoadOverlay.addPixmap(
            QtGui.QPixmap(":/actions/icons/tree_64.png"),
            QtGui.QIcon.Normal,
            QtGui.QIcon.Off,
        )
    ...
    def showLoadDialog(self, fname=None):
        ...
        if os.path.isfile(fname):
            ...
            self.lasagna.addIngredient(
                object_name=objName, kind=self.kind, data=np.asarray(data), fname=fname
            )
